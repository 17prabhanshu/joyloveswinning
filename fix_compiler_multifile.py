...
new_compile = """def compile_c_to_arduino(c_code: str, fqbn: str = "arduino:avr:uno") -> str:
    ...
    # 1. Save original firmware as a C++ file (with mocked UART/GPIO neutralized)
    c_code = c_code.replace("void GPIO_Write(uint8_t pin, bool value) {", "void MOCK_GPIO_Write(uint8_t pin, bool value) {")
    c_code = c_code.replace("void UART_Print(const char* msg) {", "void MOCK_UART_Print(const char* msg) {")
    c_code = c_code.replace("int main(void)", "int disabled_main(void)")
    
    cpp_path = os.path.join(sketch_dir, "fan_controller.cpp")
...
extern void firmware_tick(void);
